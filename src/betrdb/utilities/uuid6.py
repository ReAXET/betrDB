"""Custom UUID generator for BETR-DB"""
import secrets
import uuid
import time
...
class UUID(uuid.UUID):
    ...
    def __init__(
        self,
        hex: str = None,  # type: ignore
        bytes: bytes = None,  # type: ignore
        bytes_le: bytes = None,  # type: ignore
        fields: tuple[int, int, int, int, int, int] = None,  # type: ignore
        int: int = None,  # type: ignore
...
def uuid6(clock_seq: int = None) -> UUID:  # type: ignore
